...
class Customer(TransactionBase):
	...
	def validate(self):
		...
		if self.sales_team:
			if abs(sum(member.allocated_percentage or 0 for member in self.sales_team) - 100) > 0.00001:
				self.sales_team[-1].allocated_percentage = round(100 - sum(member.allocated_percentage or 0 for member in self.sales_team[:-1]), 2)
